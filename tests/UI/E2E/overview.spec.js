...
test.describe("cart page validation", () => {
    ...
    test("TC_01 verify URL and UI Element on Overview Page", async ({ page }) => {
        const uiElements = await overviewRef.getURLandUIElements_Overview()
    ...
    test("TC_02 get overview product details", async ({ page }) => {

        const cartProductDetails = await overviewRef.func_getAllProductDetails_Overview()
        const price = await overviewRef.getItemTotal_Overview()
        const tax = await overviewRef.getTax_Overview()
        const total = await overviewRef.getTotal_Overview()
    ...
    test("TC_05 validate summation of prices",async ({page})=>{
        const overviewPorducts = await overviewRef.func_getAllProductDetails_Overview()
        ...
        const totolPriceCalculated =await overviewRef.getItemTotal_Overview()
        expect(totalPriceSummation).toEqual(totolPriceCalculated)
    })
    test("TC_06 validate total item + tax calculation",async ({page})=>{
        const tax = await overviewRef.getTax_Overview()
        console.log(tax)
        const totalPrice = await overviewRef.getItemTotal_Overview()
         console.log(totalPrice)
        const totalIncludingTax=await overviewRef.getTotal_Overview()
